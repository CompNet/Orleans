...
#		hist(soccapind[,i],probability=TRUE,breaks=100,main=paste("Distribution of",soccap.names[i]),xlab=soccap.names[i],col="RED")
#		dev.off()
#		
...
#		pdf(file=plot.file, bg="white")
#		ecdflt(x=soccapind[sampled,i], xlab=soccap.names[i], main=paste("Complementary Cumulative Distribution of",soccap.names[i]), log="y", complementary=TRUE, col="RED", points=1000) #
#		dev.off()
#	}
#end.time <- Sys.time();
#total.time <- end.time - start.time;
...
{	for(j in 1:ncol(x))
	{	cat("[",format(Sys.time(),"%a %d %b %Y %H:%M:%S"),"] Ploting ",rolemeas.names[j]," vs. ",soccap.names[i],"\n",sep="")
		...
		pdf(file=plot.file, bg="white")
		plot(soccap.indices[sampled,i],x[sampled,j],main=paste(rolemeas.names[j],"vs.",soccap.names[i]),xlab=soccap.names[i],ylab=rolemeas.names[j],col="RED")
		dev.off()
	}
}
