...
# source("src/old_undir/p04-measures.R")
###############################################################################
library("igraph")
source("src/ecdflt.R")
...
cat("[",format(start.time,"%a %d %b %Y %H:%M:%S"),"] Cleaning data\n",sep="")
	...
	{	cat("[",format(Sys.time(),"%a %d %b %Y %H:%M:%S"),"] ..Processing col.",c,"\n",sep="")
		...
		idx.inf <- which(is.infinite(x[,c]))# | data[,c]==1.79769e+308)
		if(length(idx.inf)>0)
		{	x[idx.inf,c] <- 0
...
sampled <- sample(x=1:nrow(x),size=sample.size)
...
cat("[",format(start.time,"%a %d %b %Y %H:%M:%S"),"] Plot role measure distributions\n",sep="")
	...
	{	# histogram
		...
		hist(x[,i],probability=TRUE,breaks=100,main=paste("Distribution of",rolemeas.names[i]),xlab=rolemeas.names[i],col="RED")
		dev.off()
		...
		pdf(file=plot.file, bg="white")
		ecdflt(x=x[sampled,i], xlab=rolemeas.names[i], main=paste("Complementary Cumulative Distribution of",rolemeas.names[i]), points=1000, log="y", col="RED", complementary=TRUE) #
		dev.off()
	}
end.time <- Sys.time();
total.time <- end.time - start.time;
...
{	cat("[",format(Sys.time(),"%a %d %b %Y %H:%M:%S"),"] ..Processing role measure ",rolemeas.names[i],"\n",sep="")
	plf <- power.law.fit(x=x[sampled,i], implementation="plfit")
