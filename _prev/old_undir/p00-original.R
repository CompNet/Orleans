...
{	cat("[",format(Sys.time(),"%a %d %b %Y %H:%M:%S"),"] ..Clustering role measures for network ",network.name,"\n",sep="")
	...
	x <- cbind(participations[[network.name]],zscores[[network.name]])
	
	for(k in 2:k.max)
	{	membership <- kmeans(x=x, centers=k,  nstart=5, algorithm="Hartigan-Wong")$cluster
		qual.value <- index.DB(x=x, cl=membership, centrotypes="centroids")$DB
