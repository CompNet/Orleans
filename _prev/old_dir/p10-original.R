...
cat("[",format(start.time,"%a %d %b %Y %H:%M:%S"),"] Identifying original node roles\n",sep="")
	...
	{	if(i%%1000==0)
		...
		for(j in 1:2)
		{	# non-hubs
			if(x[sampled[i],j]<2.5)
			{	if(x[sampled[i],j+2]<0.05)
					roles[i,j] <- 1		# Ultra-Peripheral
				else if(x[sampled[i],j+2]<0.625)
					roles[i,j] <- 2		# Peripheral
				else if(x[sampled[i],j+2]<0.8)
			...
			{	if(x[sampled[i],j+2]<0.3)
					roles[i,j] <- 5		# Provincial Hub
				else if(x[sampled[i],j+2]<0.75)
...
cat("[",format(start.time,"%a %d %b %Y %H:%M:%S"),"] Plot roles using original layout\n",sep="")
	...
	{	# get extreme values for y axis
		min.y <- min(x[sampled,i])
		if(is.na(zoom))
		{	max.y <- max(x[sampled,i])
		...
		{	color <- role.colors[r]
			idx <- which(roles[,i]==r)
			points(x[sampled[idx],i+2],x[sampled[idx],i],pch=21,bg=color,cex=2)
		}
...
cat("[",format(start.time,"%a %d %b %Y %H:%M:%S"),"] Plot roles using original layout\n",sep="")
	...
	{	# get extreme values for y axis
		...
		{	idx <- which(membership==c)
			points(x[sampled[idx],i+2],x[sampled[idx],i],pch=21,bg=c,cex=2)
		}
