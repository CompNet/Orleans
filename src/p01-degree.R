...
###############################################################################
folder.data <- "data/"
file.input.soccap <- "degrees.txt"				# TODO you can possibly change that
...
cat("[",format(start.time,"%a %d %b %Y %H:%M:%S"),"] Loading degree data\n",sep="")
	...
	if(ncol(soccap.indices)==2)
		soccap.indices <- cbind(soccap.indices,soccap.indices[,1]+soccap.indices[,2])
end.time <- Sys.time();
...
cat("[",format(start.time,"%a %d %b %Y %H:%M:%S"),"] Plot degree distributions\n",sep="")
	...
	{	deg.name <- paste(degree.names[i],"-degree",sep="")
		...
		hist(soccap.indices[,i],probability=TRUE,breaks=100,main=paste("Distribution of",deg.name),xlab=deg.name,col="RED")
		dev.off()
		...
		pdf(file=plot.file, bg="white")
		ecdflt(x=soccap.indices[sampled,i], xlab=deg.name, main=paste("Complementary Cumulative Distribution of",deg.name), log="y", complementary=TRUE, col="RED", points=1000) #
		dev.off()
	}
end.time <- Sys.time();
total.time <- end.time - start.time;
...
{	deg.name <- paste("degree-",degree.names[i],sep="")
	...
	{	cat("[",format(Sys.time(),"%a %d %b %Y %H:%M:%S"),"] Ploting ",rolemeas.names[j]," vs. ",deg.name,"\n",sep="")
		...
		pdf(file=plot.file, bg="white")
		plot(soccap.indices[sampled,i],data[sampled,j],main=paste(rolemeas.names[j],"vs.",deg.name),xlab=deg.name,ylab=rolemeas.names[j],col="RED")
		dev.off()
	}
}
