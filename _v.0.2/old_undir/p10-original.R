...
cat("[",format(start.time,"%a %d %b %Y %H:%M:%S"),"] Identifying original node roles\n",sep="")
	...
	{	if(i%%1000==0)
		...
		# non-hubs
		if(x[sampled[i],1]<2.5)
		{	if(x[sampled[i],2]<0.05)
				roles[i] <- 1	# Ultra-Peripheral
			else if(x[sampled[i],2]<0.625)
				roles[i] <- 2	# Peripheral
			else if(x[sampled[i],2]<0.8)
		...
		{	if(x[sampled[i],2]<0.3)
				roles[i] <- 5	# Provincial Hub
			else if(x[sampled[i],2]<0.75)
...
cat("[",format(start.time,"%a %d %b %Y %H:%M:%S"),"] Plot roles using original layout\n",sep="")
	...
	min.y <- min(x[sampled,1])
	if(is.na(zoom))
	{	max.y <- max(x[sampled,1])
	...
	{	color <- role.colors[r]
		idx <- which(roles==r)
		points(x[sampled[idx],2],x[sampled[idx],1],pch=21,bg=color,cex=2)
	}
...
cat("[",format(start.time,"%a %d %b %Y %H:%M:%S"),"] Plot roles using original layout\n",sep="")
	...
	{	idx <- which(membership==c)
		points(x[sampled[idx],2],x[sampled[idx],1],pch=21,bg=c,cex=2)
	}
